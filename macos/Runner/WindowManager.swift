...
class WindowManager {
    ...
    func handleWindowMethods(call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "minimizeWithoutAnimation":
            minimizeWindowWithoutAnimation(result: result)
        case "hideWindow":
    ...
    /// 普通的窗口最小化方法
    private func minimizeWindow(result: @escaping FlutterResult) {
    ...
    /// 无动画的窗口最小化方法
    private func minimizeWindowWithoutAnimation(result: @escaping FlutterResult) {
        if let window = NSApplication.shared.mainWindow {
            // 无动画版本的最小化
